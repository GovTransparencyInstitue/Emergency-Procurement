...
#Dropping missings and creating a new variable for call integrity ----
italy<- italy %>% filter(!is.na(tender_indicator_INTEGRITY_CALL_FOR_TENDER_PUBLICATION))
...
ncontracts1 <- italy_disaster1 %>% filter(treatcon == 1) %>% group_by(Date, treatcon, callintegrity)%>% mutate(numberofcontracts = n()) %>% dplyr::select(Date, numberofcontracts, treatcon, disnumber, callintegrity) %>% distinct()
...
ncontracts2 <- italy_disaster2 %>% filter(treatcon == 1) %>% group_by(Date, callintegrity)%>% mutate(numberofcontracts = n()) %>% dplyr::select(Date, numberofcontracts, treatcon, disnumber, callintegrity) %>% distinct()
...
ncontracts3 <- italy_disaster3 %>% filter(treatcon == 1) %>% group_by(Date, treatcon, callintegrity)%>% mutate(numberofcontracts = n()) %>% dplyr::select(Date, numberofcontracts, treatcon, disnumber, callintegrity) %>% distinct()
...
ncontracts4 <- italy_disaster4 %>% filter(treatcon == 1) %>% group_by(Date, treatcon, callintegrity)%>% mutate(numberofcontracts = n()) %>% dplyr::select(Date, numberofcontracts, treatcon, disnumber, callintegrity) %>% distinct()
...
ncontracts5 <- italy_disaster5 %>% filter(treatcon == 1) %>% group_by(Date, treatcon, callintegrity)%>% mutate(numberofcontracts = n()) %>% dplyr::select(Date, numberofcontracts, treatcon, disnumber, callintegrity) %>% distinct()
...
model_logit<- glm(callintegrity ~ treatmentstatus + factor(newcpv) + buyer+ log_contractvalue + contractyear + contractmonth, family ="binomial", data = italy_reg)
...
model3_logit<- glm(callintegrity ~ treatmentstatus + factor(newcpv) + buyer + log_contractvalue + contractmonth, family ="binomial", data = italy_reg3)
...
model2_logit<- glm(callintegrity ~ treatmentstatus + log_contractvalue + contractyear + contractmonth + factor(newcpv) + buyer, family ="binomial", data = italy_reg2)
...
model1_logit<- glm(callintegrity ~ treatmentstatus + contractmonth + contractyear + factor(newcpv) + log_contractvalue + buyer, family ="binomial", data = italy_reg1)
...
italy_reg<-readRDS('logit_callintegrity.Rds')
#-------------------------------------------------#

#-------------------------------------------------#
saveRDS(italy_reg1, 'italy_reg1_logit_callintegrity.Rds')

italy_reg1<-readRDS('italy_reg1_logit_callintegrity.Rds')
#-------------------------------------------------#
